...
모든 검증과 미확정 결정을 해소하며 독립 리뷰 증거를 완성하세요.
-->
...
| 대상 | 명령·확인 | 결과 | 증거 |
| --- | --- | --- | --- |
| 독립 리뷰 | 원본 요구사항·raw diff·테스트 결과를 예상 결론 없이 분리된 읽기 전용 검토자에게 제공 | <통과·실패·미실행> | <검토 snapshot·검토자 수와 관점·P0~P2 발견·해소 결과·리뷰 기록> |
...
- 판정: <변경·변경 없음·결정 필요>
- 대상 파일·ID: <저장소 상대 경로와 PRD·POL·D·F ID>
