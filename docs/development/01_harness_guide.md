...
- **핵심 입력:** base `main`인 Ready PR의 정확한 current base/head Git object와 head tree, explicit same-repository source와 각각 정확히 하나인 credential 없는 canonical `origin` fetch·push URL, 같은 head의 독립 리뷰 snapshot, 같은 PR `statusCheckRollup`에 귀속된 필수 CI, repo·PR·base·head·source·`updatedAt`가 일치하고 `totalCount`와 첫 page 고유 node 수가 같으며 양쪽 page와 미해결 thread가 0개인 응답, 검증된 제목·본문·`Closes #N`, 원격 branch의 현재 OID, [`open-pull-request` finalize](../../.agents/skills/open-pull-request/SKILL.md)와 [`run-github-work-item complete`](../../.agents/skills/run-github-work-item/SKILL.md)다.

- **완료 조건:** `finalize-merge.mjs`가 검증된 repository·PR·head·제목·`updatedAt`을 plan token에 고정하고 제목을 shell 문자열이 아닌 별도 argv로 전달해 exact-head squash merge를 한 번만 실행한다. `finalize-remote-branch.mjs`가 고정한 same-repository push URL과 plan token, remote branch OID가 검증한 head와 같을 때만 그 URL로 lease/CAS 삭제해 ref 부재·병합 결과를 재조회한 뒤 같은 repository를 명시한 `complete`가 성공한다. 중단 뒤에는 `MERGED`·`mergedAt`·merge commit·exact head와 종료 참조뿐 아니라 merge의 유일한 parent=PR base, merge tree=head tree, 제목·actor와 `origin/main` first-parent 포함을 recovery mode로 재검증하되 병합 전용 CI·review thread를 다시 판정하지 않는다. `complete --dry-run --head --repo`가 same-repository PR·선점·담당자 연결을 확인한 뒤 merge를 반복하지 않고 남은 단계만 수행하며, issue worktree가 이미 없으면 clean `main` worktree에서 재개한다. 그 뒤 `finalize-local-cleanup.mjs`가 explicit repository·worktree HEAD·local ref를 검증한 identity에 결속한다. raw origin URL은 저장·출력하지 않고 fetch·push fingerprint는 plan token과 runtime canary에만 포함한다. durable core identity에는 explicit repository만 두고 archive key는 repo·URL과 무관한 stable local locator identity로 유지한다. repository 변경은 같은 namespace의 core collision으로 중단하지만 같은 repository의 canonical URL 변경은 old token을 무효화한 뒤 새 dry-run으로 기존 archive를 복구할 수 있다. 실제 root `.omc/`는 source를 삭제·이동하지 않은 채 helper-owned 새 inode sealed snapshot으로 Git common dir의 append-only generation에 보존하고 source와 copy의 content digest, durable intent·receipt와 전체 generation chain을 검증한다. final namespace 밖 `snapshot-scratch/`의 nonce root에 device/inode ownership과 durable attempt를 결속하고 exact bound scratch에서만 copy한다. nonempty 실패 candidate만 `partial`, 첫 entry 전 실패한 exact empty candidate는 `failed-empty` orphan receipt로 attempt·root·failed proof를 보존한다. 그 뒤 현재 source가 있으면 preserved generation을, 없으면 truthful empty generation을 append하며, attempt 전 중단된 empty scratch는 inert residue로만 보존한다. receipt-less preserved intent의 source가 사라져도 exact candidate가 있으면 nonempty partial은 `partial` orphan, exact empty failure는 `failed-empty` orphan, complete candidate는 preserved로 봉인한 뒤 truthful empty generation을 잇고, source와 candidate가 모두 없을 때만 fail-closed한다. mutable 원본 `.omc/`는 exact worktree root에 남겨 기존 writable FD의 후속 쓰기와 함께 보존한다. `.omc/`가 없을 때도 빈 generation을 만든다. worktree root와 Git worktree metadata는 별도 durable intent에 결속된 append-only quarantine으로 atomic no-replace 이동하며, root `.git` marker와 metadata `commondir`·`gitdir`·`HEAD` byte digest·inode를 검증하고 `git worktree remove/prune`은 호출하지 않는다. origin fingerprint와 archive proof는 identity·pending cleanup, generation intent·container, attempt, copy 시작·종료, scratch→pending, outcome, pending→current, generation receipt, quarantine intent·root·metadata·receipt와 local ref CAS의 모든 durable boundary 직전·직후에 재확인한다. root 이동 직전에는 origin canary 뒤 일반 residue scan을 마지막 bounded operation으로 실행하고, exact quarantined root와 current metadata/index를 명시한 post-move scan을 root·metadata·receipt hook 뒤와 ref CAS 직전에 반복한다. `.omc` 밖 residue는 ref와 함께 남겨 사용자가 제거·re-home할 때까지 복구를 중단하며 자동 삭제·이동·reset·stash하지 않는다. 외부 writer freeze lease가 없어 scan과 rename/CAS 사이의 완전한 동결은 보장하지 않는다. local ref CAS 직전에는 fresh full plan과 원래 plan token의 전체 일치를 요구한다. registration 부재 뒤 old-OID local ref CAS를 수행하고 모든 intent·receipt·sealed payload proof·최신 clean `main`을 확인한다.

- **대표 실패·중단 조건:** Draft·base/head·source·제목·본문·종료 참조 불일치, fork·다른 canonical fetch/push repository, multiple·credential remote URL, repository·origin fingerprint drift, 다른 PR/head에 귀속된 CI·thread, `totalCount`·양쪽 page·cursor 불완전, stale review, 실패·대기 CI, 미해결 thread, `OPEN`과 `merged-recovery` mode 혼용, squash topology·tree·actor 불일치, merge 응답 불명확 상태의 재시도, 이동·재생성돼 OID가 다른 remote branch 삭제, `complete` 전 정리, 검증한 head와 다른 local OID, root `.omc` 외 ignored·untracked·tracked 상태, 신뢰되지 않은 OMC symlink·hardlink·special file, snapshot 중 source 변경, ownership 없는 pending·current, nonempty unbound scratch, empty `partial` 또는 nonempty `failed-empty`, sealed payload proof·generation chain·archive 또는 quarantine 충돌, metadata identity 불일치, 원래 경로에 다시 생긴 bounded residue의 자동 삭제, dirty·사용자 소유 로컬 변경 또는 추측한 worktree·branch 삭제다.
